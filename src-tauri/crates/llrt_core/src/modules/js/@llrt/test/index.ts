...
import type { SocketReqMsg } from './shared';
...
class TestServer {
	...
	private tick() {
		...
		for (const id in this.workerData) {
			const workerData = this.workerData[id];
			if (
				...
				this.handleWorkerCompleted(Number.parseInt(id));
			}
		}
		...
		for (const file of this.testFiles) {
			const results = this.results.get(file);
	...
	private printResults() {
		...
		if (this.totalFailed > 0) {
			...
			for (const [file, testFailure] of sortedFilesFailed) {
				output += `\n${Color.RED_BACKGROUND(` ${file} `)}\n`;

				for (const failure of testFailure) {
					output
