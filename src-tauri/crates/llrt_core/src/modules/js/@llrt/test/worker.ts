import type { SocketReqMsg, SocketResponseMap } from './shared';
...
class TestAgent {
	private static DEFAULT_TIMEOUT_MS = Number.parseInt((process.env as any).TEST_TIMEOUT) || 5000;

	private static EMPTY_ARROW_FN_REGEX = /^(async)?\s*\(\s*\)\s*=>/m;
	private static EMPTY_FN_REGEX = /^(async)?\s*function\s*(?:[\w-]+\s*)?\(\s*\)\s*\{/m;
	...
	private createDescribe({
		...
		return (desc: string, fn: () => Promise<void>, timeout?: number) => {
			this.suiteLoadPromises.push(async () => {
				const parent: TestSuite = this.currentSuites.shift() ?? this.rootSuite;
				this.currentSuite = {
					tests: [],
					suites: [],
					parent,
				...
				const beforeLength = this.suiteLoadPromises.length;

				await fn();
				const afterLength = this.suiteLoadPromises.length;

				const items = this.suiteLoadPromises.splice(beforeLength, afterLength - beforeLength);
				if (items.length) {
					this.suiteLoadPromises.unshift(...items);
					const subSuites = Array.from({ length: items.length }).fill(this.currentSuite);
	...
	private createTestFunction({
		...
		return (desc: string, fn: () => Promise<void>, timeout?: number) => {
			const suite: TestSuite = this.currentSuite;
	...
	private async executeAsyncOrCallbackFn(
		...
		if (usesArgument) {
			await new Promise<void>((resolve, reject) => {
				const timeoutId = setTimeout(reject, timeout, timeoutMessage);
			...
			const timeoutPromise = new Promise<void>((_, reject) => {
				timeoutId = setTimeout(reject, timeout, timeoutMessage);
			});
			try {
...
const workerId = Number.parseInt(workerIdEnv || '');
const serverPort = Number.parseInt(serverPortEnv || '');
