...
impl GitPackageRegistry {
    ...
    pub fn discover_repos(&self) -> anyhow::Result<Vec<GitRepoInfo>> {
    ...
    pub fn add_repo(&self, repo_ref: GitHubRepoRef) -> anyhow::Result<GitRepoInfo> {
        let repos_root = self.github_repos_dir();
    ...
    pub fn sync_repo(&self, repo_ref: GitHubRepoRef) -> anyhow::Result<GitRepoInfo> {
        let repos_root = self.github_repos_dir();
        let repo_path = repo_ref.local_path(&repos_root);
...
impl ChordPackageRegistry {
    pub fn new(safe_handle: SafeAppHandle) -> Result<Self> {
        Ok(Self {
            git: GitPackageRegistry::new(safe_handle.path().app_cache_dir()?),
            local: LocalPackageRegistry::new(safe_handle),
        })
    }
