...
fn press(ctx: Ctx, key: String) -> rquickjs::Result<()> {
    ...
    let shortcut = Shortcut::parse(&key).or_throw_msg(&ctx, &format!("Invalid shortcut {}", key))?;
    chord_runner.shortcut.press_shortcut(shortcut, 1).or_throw_msg(&ctx, "failed to press shortcut")?;
    Ok(())
}
...
fn release(ctx: Ctx, key: String) -> rquickjs::Result<()> {
    ...
    let chord_runner = handle.chord_runner();
    let shortcut = Shortcut::parse(&key).or_throw_msg(&ctx, &format!("Invalid shortcut {}", key))?;
    chord_runner.shortcut.release_shortcut(shortcut).or_throw_msg(&ctx, "failed to release shortcut")?;
    Ok(())
}
...
fn tap(ctx: Ctx, key: String) -> rquickjs::Result<()> {
    ...
    let chord_runner = handle.chord_runner();
    let shortcut = Shortcut::parse(&key).or_throw_msg(&ctx, &format!("Invalid shortcut {}", key))?;
    chord_runner.shortcut.press_shortcut(shortcut.clone(), 1).or_throw_msg(&ctx, "failed to press shortcut")?;
    chord_runner.shortcut.release_shortcut(shortcut.clone()).or_throw_msg(&ctx, "failed to press shortcut")?;
    Ok(())
}

fn get_global_hotkey(ctx: Ctx, bundle_id: String, hotkey_id: String) -> rquickjs::Result<Option<String>> {
    let userdata = ctx.userdata::<AppUserData>().unwrap();
    let handle = &userdata.handle;
    let global_hotkey_store = handle.app_global_hotkey_store();
    let shortcut = global_hotkey_store.entries().into_iter().find_map(
        |(shortcut, entry)| {
...
fn register_global_hotkey(ctx: Ctx, bundle_id: String, hotkey_id: String) -> rquickjs::Result<Option<String>> {
    let userdata = ctx.userdata::<AppUserData>().unwrap();
    let handle = &userdata.handle;
    let global_hotkey_store = handle.app_global_hotkey_store();
...
fn set_app_needs_relaunch(ctx: Ctx, bundle_id: String, needs_relaunch: bool) -> rquickjs::Result<()> {
    let userdata = ctx.userdata::<AppUserData>().unwrap();
    let handle = &userdata.handle;
    let desktop_app_manager = handle.desktop_app_manager();
    desktop_app_manager.set_app_needs_relaunch(
        &bundle_id,
        needs_relaunch
