...
Unlike sanji's npm patches, LLRT is a **git Cargo dependency**. Apply this codemod directly against a checkout of the fork at the rev pinned in `apps/keychord-tauri/src-tauri/Cargo.toml`, then commit the result to the fork.
...
codemod jssg run --target llrt_core/build.rs -l rust --allow-fs --allow-dirty \
  /path/to/keychord/dev/improve/keychord/configs/patching/codemods/llrt/codemod.ts
```

## Run the JSSG tests

```sh
bun run --filter @keychord/dev.improve.keychord.configs.patching.codemods.llrt test
```
