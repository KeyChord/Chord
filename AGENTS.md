...
This repo is a Bun workspace monorepo (similar to [sanji](https://github.com/leonsilicon/sanji)):

- `apps/keychord-tauri` — the Tauri desktop app (Rust backend + React frontend). TanStack Router files in `src/routes/` are thin wrappers that re-export `*Page` components from route packages.
- `keychord/dev/improve/keychord/routes/**` — route packages (page UI + logic), mirroring the sanji `sanjiapp/dev/improve/sanji/routes/**` layout
- `keychord/dev/improve/keychord/configs/**` — tooling packages (e.g. patching codemods)

Run app commands from the repo root with `bun dev`, `bun build`, etc., or `cd apps/keychord-tauri` for direct access.

# Architecture

Chord is built with Tauri and uses Rust for the app backend (located in `apps/keychord-tauri/src-tauri/`) and TypeScript + React for the app frontend (located in `apps/keychord-tauri/src/`).
...
Rust shares state with React using "observables", which are provided by the [observable-property](https://crates.io/crates/observable-property) crate, all of which are located in `apps/keychord-tauri/src-tauri/src/observables`). You should use an observable for any state you want accessible to the frontend. Under the hood, we simply call Tauri's `invoke` function to update it whenever the state in an observable changes, and keep it synced in React using a [custom hook](./apps/keychord-tauri/src/utils/state.ts) that wraps `listen`.

On the Rust side, observable state can be accessed from anywhere via `handle.observable_state::<MyObservable>()`. However, they are always owned by state singletons: app-level structs which are registered using [Tauri's state management](https://v2.tauri.app/develop/state-management/) via [`app.manage::<MyStateType>`](./apps/keychord-tauri/src-tauri/src/setup.rs). In `setup.rs`, we create a single instance of each of these state singletons, as well as a single instance of each observable.
...
All the state singletons are defined inside of the [app/](./apps/keychord-tauri/src-tauri/src/app) folder in `src-tauri/`. We use [a macro](./apps/keychord-tauri/src-tauri/src/app/mod.rs) to make all of them exposed on the `handle` directly (e.g. `handle.chord_package_manager()` instead of handle.state::<ChordPackageManager>()`).
